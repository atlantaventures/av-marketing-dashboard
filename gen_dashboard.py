...
<main class="main">
  ...
  <section class="section" id="section-goals">
    ...
    <div class="notes-panel" id="notes-goals"><div class="notes-panel-label">Notes &amp; Decisions</div><textarea class="notes-textarea" placeholder="Add notes, decisions, or context about goals…" oninput="saveNote('goals',this.value)"></textarea></div>
  </section>
  ...
  <section class="section" id="section-web">
    ...
    <div class="notes-panel" id="notes-web"><div class="notes-panel-label">Notes &amp; Decisions</div><textarea class="notes-textarea" placeholder="Add notes, decisions, or context about web…" oninput="saveNote('web',this.value)"></textarea></div>
  </section>
  <div class="divider"></div>
  ...
  <section class="section" id="section-newsletters">
    ...
    <div class="notes-panel" id="notes-newsletters"><div class="notes-panel-label">Notes &amp; Decisions</div><textarea class="notes-textarea" placeholder="Add notes, decisions, or context about newsletters…" oninput="saveNote('newsletters',this.value)"></textarea></div>
  </section>
  <div class="divider"></div>
  ...
  <section class="section" id="section-content">
    ...
    <div class="notes-panel" id="notes-content"><div class="notes-panel-label">Notes &amp; Decisions</div><textarea class="notes-textarea" placeholder="Add notes, decisions, or context about content…" oninput="saveNote('content',this.value)"></textarea></div>
  </section>
  <div class="divider"></div>
  ...
  <section class="section" id="section-events">
    ...
    <div class="notes-panel" id="notes-events"><div class="notes-panel-label">Notes &amp; Decisions</div><textarea class="notes-textarea" placeholder="Add notes, decisions, or context about events…" oninput="saveNote('events',this.value)"></textarea></div>
  </section>
  <div class="divider"></div>
  ...
  <section class="section" id="section-context">
    ...
    <div class="ctx-add-form">
      <div class="ctx-add-title">Add entry for <span id="ctx-current-period"></span></div>
      <div class="ctx-form-row">
        <div class="ctx-form-label">Decisions made</div>
        <textarea class="ctx-form-textarea" id="ctx-decisions" placeholder="What did we decide this month? One decision per line."></textarea>
      </div>
      <div class="ctx-form-row">
        <div class="ctx-form-label">Things to watch</div>
        <textarea class="ctx-form-textarea" id="ctx-watch" placeholder="What are we monitoring going forward? One item per line."></textarea>
      </div>
      <div class="ctx-form-row">
        <div class="ctx-form-label">Additional notes</div>
        <textarea class="ctx-form-textarea" id="ctx-extra" placeholder="Anything else worth capturing for future reference…"></textarea>
      </div>
      <button class="ctx-save-btn" onclick="saveContextEntry()">Save Entry</button>
    </div>
  ...
  months:{
    "2026-07":{
      web:{
        ...
        engagement_rate:{v:null,mom:null,yoy:null},form_submissions:{v:null,mom:null,yoy:null},event_count:{v:null,mom:null,yoy:null},
        traffic:[
        ...
        note:"form_submissions and event_count not captured this cycle (ga4_pull.py doesn't collect these fields yet); engagement_rate not available."
      },
      social:{
        linkedin:{
...
function renderEvents(){
  ...
  const [pyear,pmonth]=DATA.meta.period_key.split('-');
...
function renderContextLog(){
  ...
  function renderEntries(){
    ...
    html+=userEntries.map(e=>`<div class="ctx-entry" style="border-color:var(--orange-pale)">
      <div class="ctx-entry-header">
        <span class="ctx-entry-period">${e.period} <span style="font-size:11px;font-weight:400;color:var(--gray-400)">— added by team</span></span>
...
// ── INIT ─────────────────────────────────────
function init(){
  renderGoals();renderWeb();renderSocial();renderSocialTrend();renderLIIGTrend();renderNewsletters();renderBlogs();renderBlogTrend();renderSubstack6Mo();renderEvents();renderNarratives();renderContextLog();loadNotes();
