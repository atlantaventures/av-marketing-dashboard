...
const DATA = {
  ...
  narrative:{
    ...
    newsletters:{
      read:"• Mailchimp has no data this month — the Mailchimp MCP connector appears disconnected in this session, and no manual fallback numbers were available in time for this pull; this is a full gap, not a no-send month\\n• LinkedIn Newsletter sent 2 articles: \\\"The Perlant Named Best Wine Bar in Atlanta\\\" (Aug 5: 3,210 impressions, 182 engagements, 1,581 article views, 43% email open rate) and \\\"Hot Off The Press: Office Hours Is This Thursday\\\" (Aug 12: 1,070 impressions, 54 engagements, 1,490 article views, 43% email open rate)\\n• Combined impressions of 4,280 are up 94.6% vs July's 2,199, and combined engagements of 236 are up sharply from July's 25\\n• LinkedIn Newsletter subscribers grew to 3,605 (+2.3% vs July's 3,523), extending the unbroken monthly growth streak into an 8th month\\n• LinkedIn's own \\\"Article totals\\\" overview card for the period shows 4,337 impressions and only 124 engagements — the engagements figure doesn't reconcile with the sum of the two per-article breakdowns (236); the per-article numbers are used here since they match the Metricool pull exactly, but this is worth flagging as a LinkedIn reporting inconsistency, not a data error on this end",
      rec:"• Resolve the Mailchimp connector gap before next month — either reconnect the MCP integration or establish a reliable manual pull process, since a second consecutive missing month would start to look like the multi-month email silence flagged earlier this year\\n• The Article totals vs per-article engagement mismatch (124 vs 236) is worth a quick check in LinkedIn's own dashboard — it may just be a different counting window (e.g. totals capped to the exact calendar month while per-article stats run from send date), but it's been showing up more than once now\\n• Keep the 2-articles/month cadence going — the subscriber growth streak has now held for 8 consecutive months"
    },
    content:{
